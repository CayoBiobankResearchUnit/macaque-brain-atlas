...
checkpoint.file = file.path('rds',paste0(prefix,'_all_marker_peaks_combined.rds'))
if (!file.exists(checkpoint.file) || !use.checkpoint) {
	scanpy = readRDS(file.path('rds',paste0(prefix,'_marker_peaks.rds')))
	...
	seurat = do.call(rbind,lapply(file.path('rds',list.files('rds',pattern=paste0(prefix,'_marker_peaks_seurat_all_class[0-9]+.rds'))),readRDS))
...
p = ggplot(methods.merged.pass,aes(log10(pts),log10(pct.1))) +
	...
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratPercExpr_v_scanpyPercExpr.pdf')),useDingbats=FALSE))
...
p = ggplot(methods.merged.pass,aes(log10(pts_rest),log10(pct.2))) +
	...
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratPercRest_v_scanpyPercRest.pdf')),useDingbats=FALSE))
...
#	coord_equal() +
	...
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratP_v_scanpyAbsLR.pdf')),useDingbats=FALSE))
...
#	coord_equal() +
	...
	ylab(expression('Coefficient'[Scanpy])) +
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratP_v_scanpyLR.pdf')),useDingbats=FALSE))
...
#	coord_equal() +
	...
	ylab(expression('|Coefficient'[Scanpy]*'|')) +
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratP_v_scanpyAbsLR-filtered.pdf')),useDingbats=FALSE))
...
#	coord_equal() +
	...
	ylab(expression('Coefficient'[Scanpy])) +
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratP_v_scanpyLR-filtered.pdf')),useDingbats=FALSE))
...
p = ggplot() +
	...
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratFC_v_scanpyFC.pdf')),useDingbats=FALSE))
...
p = ggplot(subset(methods.merged.pass,pct.1>=marker.expr.threshold),aes(avg_log2FC,logfoldchanges,color=log10(pct.1))) +
	...
	ylab(expression('log'[2]*'FC'[Scanpy])) +
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratFC_v_scanpyFC-filtered.pdf')),useDingbats=FALSE))
...
p = ggplot() +
	...
	ylab(expression('Coefficient'[Scanpy])) +
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratFC_v_scanpyLR.pdf')),useDingbats=FALSE))
...
p = ggplot(subset(methods.merged.pass,pct.1>=marker.expr.threshold),aes(avg_log2FC,logreg_score,color=log10(pct.1))) +
	...
	ylab(expression('Coefficient'[Scanpy])) +
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratFC_v_scanpyLR-filtered.pdf')),useDingbats=FALSE))
...
#	coord_equal() +
	...
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_seuratP-qqplot.pdf')),useDingbats=FALSE))
...
methods.merged.pass = do.call(rbind,lapply(split(methods.merged.pass,methods.merged.pass$cluster),function(x) {
	logreg.threshold = median(subset(x,pts>=marker.expr.threshold & avg_log2FC > 0 & logreg_score > 0 & p_val_adj < fdr.cutoff)$logreg_score)
	...
	x$sig = factor(
		with(x,ifelse(
			...
			'not accessible',
			ifelse(
				avg_log2FC > 0 & logreg_score > 0 & p_val_adj < fdr.cutoff,
				'marker peak (Seurat)',
...
#		legend.title=element_text(size=12),
	...
	ylab(expression('Coefficient'[Scanpy])) +
	ggtitle('all cells')
suppressMessages(ggsave(p,file=file.path('figures/markerpeaks',paste0(prefix,'_all_marker_peaks.pdf')),useDingbats=FALSE))

dir.create('stats/markerpeaks',showWarnings=FALSE)
dir.create('stats/markerpeaks/bed',showWarnings=FALSE)

methods.merged.pass$cluster_number = factor(match(methods.merged.pass$cluster,cell.levels),levels=sort(unique(match(methods.merged.pass$cluster,cell.levels))))
...
for (i in levels(methods.merged.sig$cluster_number)) {
	...
	if (!nrow(this)) {
		...
		write.table(
			this,
			file=file.path('stats/markerpeaks/bed',paste0(prefix,'_all_class',i,'_marker_peaks.bed')),
...
# methods.merged.markers = subset(methods.merged.pass,logreg_score > 0 & p_adj < 0.05)
