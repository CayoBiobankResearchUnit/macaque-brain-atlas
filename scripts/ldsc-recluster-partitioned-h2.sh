...
ldsc/ldsc.py \
	...
	--ref-ld-chr stats/ldsc/${bed_prefix}/class${cell_class}_cluster${cell_cluster}.,data/1000G/${baseline_folder}/${baseline_prefix} \
	--frqfile-chr data/1000G/1000G_Phase3_frq/1000G.EUR.QC. \
	--overlap-annot --print-cov --print-coefficients --print-delete-vals \
	--out stats/ldsc/${bed_prefix}/scores/class${cell_class}_cluster${cell_cluster}-${phenotype_code}

# Write summary stats
python scripts/ldsc-recluster-partitioned-h2-summarize.py $bed_prefix $cell_class $cell_cluster $phenotype
