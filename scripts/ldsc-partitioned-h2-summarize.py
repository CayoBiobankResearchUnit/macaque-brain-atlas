...
results_file = os.path.join('stats/ldsc/'+bed_prefix+'/scores','class'+str(cell_class)+'-'+phenotype_code+'.results')
...
h2_results = pd.concat([
	pd.DataFrame(
		[[cell_class_name,phenotype_code,snp_count,h2,h2_se]],
		columns=['Cell_class','Phenotype','SNP_count','h2','h2_std_error']
	),
...
h2_results.index = ['class'+str(cell_class)+'-'+phenotype_code]
