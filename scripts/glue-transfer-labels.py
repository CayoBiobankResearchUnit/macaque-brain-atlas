...
# scglue.data.transfer_labels(rnaref,ataqry,'rnasubtype',use_rep='X_glue',key_added='glue_subtype',n_jobs=4)
# ataqry.obs.to_csv('stats/clusters/'+at_prefix+'-cellsubtype-predictions.txt',sep='\t',header=True,index=True)
# scglue.data.transfer_labels(rnaref,rnaqry,'rnasubtype',use_rep='X_glue',key_added='glue_subtype',n_jobs=4)
# rnaqry.obs.to_csv('stats/clusters/'+rn_prefix+'-cellsubtype-predictions.txt',sep='\t',header=True,index=True)
...
scglue.data.transfer_labels(rnaref,rnaqry_ex,'rnatype',use_rep='X_glue',key_added='glue_type',n_jobs=ncpu)
rnaqry_ex.obs.to_csv('stats/clusters/'+rn_prefix+'ext-celltype-predictions.txt',sep='\t',header=True,index=True)
push_status(prefix+' transfer_labels rna_ex')

scglue.data.transfer_labels(rnaref,rnaqry_in,'rnatype',use_rep='X_glue',key_added='glue_type',n_jobs=ncpu)
rnaqry_in.obs.to_csv('stats/clusters/'+rn_prefix+'int-celltype-predictions.txt',sep='\t',header=True,index=True)
push_status(prefix+' transfer_labels rna_in')

scglue.data.transfer_labels(rnaref,ataqry,'rnatype',use_rep='X_glue',key_added='glue_type',n_jobs=ncpu)
ataqry.obs.to_csv('stats/clusters/'+at_prefix+'-celltype-predictions.txt',sep='\t',header=True,index=True)
push_status(prefix+' transfer_labels atac')
